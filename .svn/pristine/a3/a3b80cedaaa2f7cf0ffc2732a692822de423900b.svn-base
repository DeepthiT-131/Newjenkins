...
export function ValidateInterTransfer(){
    return (formGroup: FormGroup) => {
        ...
        let dynControl = formGroup.controls['other'] as FormArray;
        let sumCount =0;
        let arr=[];
        ...
        for(let i=0;i<dynControl.length;i++){
            ...
            let val=formGroup['toLocation'] +' '+formGroup['toArea'] +' '+formGroup['toSite'] ;
        ...
        if(isRepeatLoc && availQuantity < sumCount){
            dynControl.setErrors({ repeatLocation: true, exceedQty: true});
        }
        else if(isRepeatLoc){
